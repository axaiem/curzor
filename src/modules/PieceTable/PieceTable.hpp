...
class PieceTable : public TextBuffer
{
    ...
    void insert (int idx, std::string text)override{
        std::unique_ptr<Operation> addition = std::make_unique<AddTextPieceTable>(m_tablePieces, idx, text, m_changes);
        addition->run();
        updateOps(std::move(addition));
    }

    void remove (int idx, int length) override{
        std::unique_ptr<Operation> deletion = std::make_unique<DeleteTextPieceTable> (m_tablePieces, idx, length);
        deletion->run();
